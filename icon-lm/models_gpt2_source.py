...
# this code is adapted from https://huggingface.co/transformers/v3.0.2/_modules/transformers/modeling_gpt2.html#GPT2Model
# here are the changes:
# 1. remove some code that I don't need for clarity, e.g. prune_heads, GPT2DoubleHeadsModel, some docstrings
# 2. add the option to use custom 2D mask instead of the default causal mask, see _attn in Attention and forward in GPT2Model.
# 3. delete the position embedding and token type embedding. You have to add them outside of the model.
...
class Attention(nn.Module):
    ...
    def _attn(self, q, k, v, attention_mask=None, head_mask=None, output_attentions=False):
        ...
        if attention_mask is not None:
            # Apply the given attention mask
            w = torch.where(attention_mask.bool(), w, self.masked_bias.to(w.dtype))
        else:
...
GPT2_INPUTS_DOCSTRING = r"""
    Args:
        ...
        attention_mask (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, sequence_length, sequence_length)`, `optional`, defaults to :obj:`None`):
            Different from the source code, 2D mask, will replace the causal mask if given. 
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    Return:
        ...
        if attention_mask is not None:
            ...
            # this attention mask is different from the source code.
